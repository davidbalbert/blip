...
func NewMachOGenerator() *MachOGenerator {
	return &MachOGenerator{}
}

func (m *MachOGenerator) SetTextData(data []byte) {
	m.textData = data
}
...
func (m *MachOGenerator) Generate() []byte {
	...
	symOffset := textOffset + uint32(len(m.textData))
	...
	binary.LittleEndian.PutUint64(segCmd[32:], uint64(len(m.textData))) // vmsize
	binary.LittleEndian.PutUint64(segCmd[40:], uint64(textOffset)) // fileoff
	binary.LittleEndian.PutUint64(segCmd[48:], uint64(len(m.textData))) // filesize
	...
	binary.LittleEndian.PutUint64(segCmd[112:], uint64(len(m.textData))) // size
	...
	// Text data
	result = append(result, m.textData...)
